...
\dfm{I think this paragraph needs a little more... e.g. This is all good and well when the inclination, rotation period, and limb darkening are known, so the data can be uniquely projected. But that is no longer true when these are parameters. Or is there more to it?}
...
\begin{align}
    \label{eq:linlstsq}
    \hat{\mathbf{y}} & \stackrel{?}{=} \left(\pmb{\mathcal{A}}^\top\,\pmb{\mathcal{A}}\right)^{-1}\,\pmb{\mathcal{A}}^\top\,\left(\mathbf{f} - \mathbf{1}\right)
    \quad.
\end{align}
%
But, since the flux operator $\pmb{\mathcal{A}}$ is low-rank, the model is underdetermined, so the above operation is not defined and, equivalently, the Fisher information matrix
%
\begin{align}
    \label{eq:fisher-info}
    \pmb{\mathcal{I}}(\mathbf{y}) & = \frac{1}{\sigma^2} \, \pmb{\mathcal{A}}^\top\,\pmb{\mathcal{A}}
\end{align}
%
is singular.
In Equation~(\ref{eq:fisher-info}), $\sigma$ is the flux measurement uncertainty.
There exist many methods and procedures for linear regression for underdetermined models, but all solutions amount to imposing stronger assumptions about the model in order to break the degeneracies.

One standard method is regularized least squares where Equation~(\ref{eq:linlstsq}) becomes
%
\begin{align}
    \hat{\mathbf{y}}_\lambda & \equiv \left(\pmb{\mathcal{A}}^\top\,\pmb{\mathcal{A}} + \lambda\,\mathbf{I}\right)^{-1}\,\pmb{\mathcal{A}}^\top\,\left(\mathbf{f} - \mathbf{1}\right)
\end{align}
%
where $\mathbf{I}$ is the identity matrix and $\lambda$ is a parameter controlling the strength of the regularization.
In a Bayesian context, this can be interpreted as placing a Gaussian prior with inverse variance $\sigma^2\,\lambda$ on the spherical harmonic coefficients.

In the limit of infinitesimal regularization, it can be demonstrated \citep[see][for example]{Hogg2021} \dfm{(TODO: I think I know how to prove this)} that
%
...
It is interesting to quantify the information content of a light curve as a function of spherical harmonic coefficients.
While Equation~(\ref{eq:fisher-info}) provides a compact expression for the Fisher information matrix, the most important feature that we want to quantify is the covariance between coefficients.
This is most naturally captured by the covariance matrix of the likelihood function for the spherical harmonic coefficients:
%
\begin{align}
    \pmb{\Sigma}_\lambda & = \sigma^2\,\left(\pmb{\mathcal{A}}^\top\,\pmb{\mathcal{A}} + \lambda\,\mathbf{I}\right)^{-1} \quad.
\end{align}
%
Below, we use this to quantify the width of the likelihood for each spherical harmonic coefficient by plotting the ``relative variance'' $S$, which we define as the diagonal of the matrix $\pmb{\Sigma}_\lambda$ in units of $\lambda / \sigma^2$ in the limit where $\lambda \to 0_+$.
This choice of units constrains $0 \le S \le 1$ and allows easy comparisons between the information content of different datasets without being sensitive to the specific modeling assumptions.
Under this definition, when $S$ approaches one for a particular coefficient, this means that the likelihood function provides no information about that coefficient, while $S = 0$ indicates that that coefficient can be perfectly measured.

This relative variance $S$ can be interpreted either as the Cram\'er--Rao lower bound on the variance of an estimator for a spherical harmonic \citep[e.g.,][]{Li2012}, although our specific choice of units are somewhat non-standard.
Similarly, in a Bayesian context, this is the ``posterior shrinkage'', the ratio of the posterior variance to the prior variance in the limit of infinite prior variance \citep[e.g.,][]{Betancourt2018}.
...
Moreover, our constraints on the information content of ensembles of light curvesare
