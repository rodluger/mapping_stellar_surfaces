...
problem of inferring the properties of a stellar surface from its light curve.
...
\xxx{This paper is also similar to \citet{Walkowicz2013} and \citet{Basri2018},
    but we approach things from a purely theoretical point of view, as opposed
    to the more empirical, qualitative approach of those two papers.}

This paper is the first in a series dedicated to the development of
...
stellar light curves, which is the subject of Paper II \citep{PaperII}.
...
\begin{figure}[t!]
    \begin{centering}
        ...
        \oscaption{rank}{%
            Rank and nullity of the flux operator. \xxx{More info.}
            \label{fig:rank}
        }
    \end{centering}
\end{figure}
...
\xxx{Briefly mention the chopping: odd modes don't increase the rank. Always
    in the null space.}
...
\begin{figure}[p!]
    \begin{centering}
        ...
        \\[1em]
        \includegraphics[width=\linewidth]{figures/nullspace_preimage_0.pdf}
        \\[1em]
        \includegraphics[width=\linewidth]{figures/nullspace_preimage_1.pdf}
        \\[1em]
        \includegraphics[width=\linewidth]{figures/nullspace_preimage_2.pdf}
        \\[1em]
        \includegraphics[width=\linewidth]{figures/nullspace_preimage_3.pdf}
...
\xxx{However, there are spurious features...}
...
\xxx{Talk about priors people use, like positivity, restrictions on
    shape and number of spots, etc. These are nonlinear, harder to
    constrain how exactly they change the nullspace. But still most
    of the info is coming from the prior, so it's important to get it
    right.}
...
\xxx{To better understand which modes we can and cannot constrain...}
...
\xxx{Discuss Figure~\ref{fig:nullspace_ensemble_single}, and how it shows
    the modes we can learn and those we can't. Can't learn odd modes, can't
    usually learn much about high degree stuff. Variance is due to different
    inclinations. What if we could harness that?}
...
Figure~\ref{fig:nullspace_preimage_ensemble} shows \xxx{...}
...
\begin{figure}[p!]
    \begin{centering}
        ...
        \\[1em]
        \includegraphics[width=\linewidth]{figures/nullspace_preimage_ensemble_0.pdf}
        \\[1em]
        \includegraphics[width=\linewidth]{figures/nullspace_preimage_ensemble_1.pdf}
        \\[1em]
        \includegraphics[width=\linewidth]{figures/nullspace_preimage_ensemble_2.pdf}
        \\[1em]
        \includegraphics[width=\linewidth]{figures/nullspace_preimage_ensemble_3.pdf}
        \oscaption{nullspace_preimage_ensemble}{%
            Same as Figure~\ref{fig:nullspace_preimage}, \xxx{...}
...
light curve problem.
%
\xxx{Talk about the next paper a bit, and how we can cast the mapping
    problem approximately as a GP problem where we solve for the parameters
    controlling the mean and covariance structure of the ensemble of
    light curves.}
%
In the context of
...
\xxx{Again, this paragraph needs a bit more motivation.}
...
\xxx{VERY IMPORTANT for this paper. The variance of the light curves, which scales their
...
\xxx{In the next paper} we show that in relative photometry there exists a
