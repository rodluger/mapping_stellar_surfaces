...
a leading tool for modeling correlated signals in astronomical time series datasets,
especially photometric time series
\citep[e.g.,][]{RasmussenWilliams2005,BrewerStello2009,Luger2016,ForemanMackey2017,Angus2018}
and radial velocities (ADD CITATIONS).
GPs are popular models for these applications because they allow marginalization
over a stochastic noise process specified only by its autocorrelation structure
or kernel.
This is relatively straightforward to interpret and in some specific cases it
can be related to physically motivated models (CITE SOME THINGS).
Furthermore, there are several popular open source implementations that allow
efficient evaluation of these models, and these have been widely demonstrated
to be useful effective models for the time series when the stochastic
variability is primarily a nuisance.
...
If, however, one hopes to make inferences about the stellar surface itself,
GP models, as they are typically applied, are not so useful.
...
Specifically, in all but a few restricted cases, there is no first principles
relationship between the descriptive parameters of a typical GP model
(see section WHATEVER below)
and the physical properties of the stellar surface surface that is being observed.
For instance, it may be tempting to interpret the amplitude of the variability
as some measure of the spot contrast or the total number of spots,
or the timescale of the variations as the
