...
Over the past two decades, Gaussian processes (GPs) have gained traction as
a leading tool for modeling correlated signals in astronomical datasets
\citep[][]{RasmussenWilliams2005}.
...
Significant effort has thus been spent on theoretical and numerical approaches to understanding
magnetohydrodynamics (MHD) in stars~\citep[e.g.,][]{Miesch2009}.
Theoretical progress is challenging; MHD equations are nonlinear, since
magnetism often emerges in regions of highly developed turbulence. There is a
large separation of scales between the size of a star and the
magnetic/thermal/momentum diffusion length-scales, and so numerical
simulations at realistic resolutions remain out of reach. As a result
there is still significant theoretical uncertainty over how dynamos
manifest in stars of different masses, how magnetism interacts with rotation,
and how star spots form \citep{Yadav2015,Weber2016}. Because of the
significant impacts of magnetism on every branch of stellar astrophysics,
these difficulties make new constraints from observations especially valuable.

\xxx{MITIGATING EXOPLANET SYSTEMATICS} \citep{Rackham2018}.
...
in the literature
has been to explicitly \emph{forward model} the stellar surface and its effect on the light curve, then
...
\xxx{REMOVE THESE EXAMPLES; NOT NEEDED.}
...
We would like to thank David W. Hogg, Michael Gully-Santiago, Adam Jermyn, and the
